...
public class LdaGibbsSampler {
	 ...
	 * Many alternatives are possible, I chose to perform random assignments with
	 * equal probabilities<br>
	 * 随机初始化状态
	 *
	 * @param K
	...
	/**
	 * Main method: Select initial state ? Repeat a large number of times: 1. Select
	 * an element 2. Update conditional on other elements. If appropriate, output
	 * summary for each run.<br>
	 * 采样
	 *
	 * @param K
	...
	/**
	 * Sample a topic z_i from the full conditional distribution: p(z_i = j | z_-i,
	 * w) = (n_-i,j(w_i) + beta)/(n_-i,j(.) + W * beta) * (n_-i,j(d_i) +
	...
	/**
	 * Retrieve estimated document--topic associations. If sample lag > 0 then the
	 * mean value of all sampled statistics for theta[][] is taken.<br>
	 * 获取文档——主题矩阵
	 *
	...
	/**
	 * Retrieve estimated topic--word associations. If sample lag > 0 then the mean
	 * value of all sampled statistics for phi[][] is taken.<br>
	 * 获取主题——词语矩阵
	 *
	...
	/**
	 * create a string representation whose gray value appears as an indicator of
	 * magnitude, cf. Hinton diagrams in statistics.
	 *
	 * @param d
	 *            value
	 * @param max
